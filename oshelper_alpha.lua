...
local ints = {
	...
	timestamp__fontsize = imgui.ImInt(cfg.timestamp.fontsize),
...
function main()
    ...
    if checkboxes.hello.v then
			if ints.active.v == 0 then
				msg('Авторы: '..color..'deveeh'..textcolor..' , '..color..'casparo'..textcolor..' и '..color..'STANLEY'..textcolor..'. Команда активации: '..color..'/oshelper') 
			end
			if ints.active.v == 1 then
				msg('Авторы: '..color..'deveeh'..textcolor..' , '..color..'casparo'..textcolor..' и '..color..'STANLEY'..textcolor..'. Чит-код: '..color..cfg.settings.cheatcode) 
			end
		end
        ...
        imgui.Begin('OS Helper v'..thisScript().version, frames.window, imgui.WindowFlags.NoResize)
			...
			if menu == 5 then
				...
				if checkboxes.timestate then
					...
					if imgui.InputInt('##Fontsize', ints.timestamp__fontsize, 1, 1) then 
						if ints.timestamp__fontsize.v < 1 then 
							ints.timestamp__fontsize.v = 1 
						elseif ints.timestamp__fontsize.v > 25 then
							ints.timestamp__fontsize.v = 25 
						end 
						cfg.timestamp.fontsize = ints.timestamp__fontsize.v
						font = renderCreateFont("Arial", cfg.timestamp.fontsize, 5) 
					end
					imgui.PopItemWidth()
			...
			imgui.CenterText(fa.ICON_FA_INFO.. u8"  Инфо-панель  "..fa.ICON_FA_INFO) 
				imgui.Text(u8"Статус: ") imgui.SameLine()
					if imgui.ToggleButton(u8'##Статус: ', checkboxes.doppanel) then
				...
				imgui.Text(u8(cfg.infopanel.doppanel and 'Активна' or 'Неактивна'))
					if imgui.Button(u8'Позиция', imgui.ImVec2(200, 25)) then
						lua_thread.create(function ()
							...
							sampSetCursorMode(4)
							sampAddChatMessage('Нажмите {FF8000}ПРОБЕЛ{FFFFFF} для сохранения позиции.')
							while checkCursor do
								local cX, cY = getCursorPos()
									...
									if isKeyDown(32) then
										...
										if inicfg.save(cfg, "OSHelper.ini") then sampAddChatMessage('Позиция инфо-панели сохранена!') end
									end
								wait(0)
							end
						end)
					end
				if imgui.ToggleButton(u8"##nickid", checkboxes.nickact) then cfg.infopanel.nickact = checkboxes.nickact.v end imgui.SameLine() imgui.Text(u8"Отображать ник и id")
				if imgui.ToggleButton(u8"##Отображать время", checkboxes.timeact) then cfg.infopanel.timeact = checkboxes.timeact.v end imgui.SameLine() imgui.Text(u8"Отображать время")
				if imgui.ToggleButton(u8"##Отображать день недели", checkboxes.daysact) then cfg.infopanel.daysact = checkboxes.daysact.v end imgui.SameLine() imgui.Text(u8"Отображать день недели")
				if imgui.ToggleButton(u8"##Отображать колличество кадров", checkboxes.fpsact) then cfg.infopanel.fpsact = checkboxes.fpsact.v end imgui.SameLine() imgui.Text(u8"Отображать колличество кадров")
				if imgui.ToggleButton(u8"##Отображать пинг", checkboxes.pingact) then cfg.infopanel.pingact = checkboxes.pingact.v end imgui.SameLine() imgui.Text(u8"Отображать пинг")
				if imgui.ToggleButton(u8"##Отображать ID своего скина", checkboxes.skinact) then cfg.infopanel.skinact = checkboxes.skinact.v end imgui.SameLine() imgui.Text(u8"Отображать ID своего скина")
				if imgui.ToggleButton(u8"##Отображать целостность броника", checkboxes.armouract) then cfg.infopanel.armouract = checkboxes.armouract.v end imgui.SameLine() imgui.Text(u8"Отображать целостность броника")
				if imgui.ToggleButton(u8"##Отображать колличество ХП", checkboxes.hpact) then cfg.infopanel.hpact = checkboxes.hpact.v end imgui.SameLine() imgui.Text(u8"Отображать колличество ХП")
			end
			imgui.EndChild()
		imgui.End()
	end
